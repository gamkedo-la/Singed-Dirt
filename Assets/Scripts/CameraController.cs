...
public class CameraController : MonoBehaviour {
    ...
    public void WatchNuke() {
        ...
        ChangeShakeAmount(0.1f);
        StartCoroutine(NukeSceneLoop());
    }

    private IEnumerator NukeSceneLoop() {
        yield return new WaitForSeconds(0.75f);
        ChangeShakeAmount(0.2f);

        yield return new WaitForSeconds(0.5f);
        dampTime = 3.5f;
        rotationDampTime = 0.0125f;
        desiredPosition = nukeLocation.position;

        yield return new WaitForSeconds(0.75f);
        desiredRotation = nukeLocation.rotation;

        yield return new WaitForSeconds(7f);
        dampTime = 0.075f;
        rotationDampTime = 0.075f;
        StopCoroutine(NukeSceneLoop());
    }
    ...
    IEnumerator ShakeCameraLoop(float amount, float decreaseFactor) {
        ...
        StopCoroutine(ShakeCameraLoop(amount, decreaseFactor));
    }
